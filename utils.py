...
def cal_oov(ch_file, en_file, ch_vocab, en_vocab):
  chvocab = [line.split()[0] for line in open(ch_vocab, 'r').read().splitlines()]
  ch_token2idx = {token: idx for idx, token in enumerate(chvocab)}
  envocab = [line.split()[0] for line in open(en_vocab, 'r').read().splitlines()]
  en_token2idx = {token: idx for idx, token in enumerate(envocab)}

  ch_lines = open(ch_file, 'r').readlines()
  oov_ch = 0
  #ch_oov = []
  for idx, line in enumerate(ch_lines):
    for item in line.strip().split(' '):
      if item not in ch_token2idx:
        oov_ch += 1
        #ch_oov.append(item)
  en_lines = open(en_file, 'r').readlines()
  oov_en = 0
  #en_oov = []
  for idx, line in enumerate(en_lines):
    for item in line.strip().split(' '):
      if item not in en_token2idx:
        oov_en += 1
        #en_oov.append(item)

  print(f'file: {ch_file}, length: {len(ch_lines)}, oov_ch: {oov_ch}, average is {oov_ch / len(ch_lines)}')
  print(f'file: {en_file}, length: {len(en_lines)}, oov_en: {oov_en}, average is {oov_en / len(en_lines)}')
